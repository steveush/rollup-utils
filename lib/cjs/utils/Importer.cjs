...
const getEntryInfo = require('./getEntryInfo.cjs');
const getImportedIds = require('./getImportedIds.cjs');
const isFilterPattern = require('./isFilterPattern.cjs');
const isFunction = require('./internal/isFunction.cjs');
const isNonNullable = require('./internal/isNonNullable.cjs');
const isPOJO = require('./internal/isPOJO.cjs');
const isString = require('./internal/isString.cjs');
const hasKeys = require('./internal/hasKeys.cjs');
...
const IMPORTER_DEFAULTS = Object.freeze({
  ...
  types: {
    default: (ctx, module) => {
      return new MagicString(module.code).replaceAll(/`/g, "\\`").prepend("export default `").append("`;");
    }
  }
});
...
class Importer {
  ...
  constructor() {
    ...
    if (!isFilterPattern(this.options.include)) throw new Error("The `include` option is required.");
    this.#matches = pluginutils.createFilter(this.options.include, this.options.exclude);
    if (!isFunction(this.#matches)) throw new Error("Unexpected result from a call to `createFilter`. Check the values passed for the `include` and `exclude` options are valid filter patterns.");
    this.#cache = new Map();
  }
  ...
  outputOptions(ctx, options) {
    ...
    if (isFunction(options.sourcemapIgnoreList)) {
      ...
      options.sourcemapIgnoreList = (relativeSourcePath, sourcemapPath) => {
        return original(relativeSourcePath, sourcemapPath) || this.matches(path.resolve(relativeSourcePath));
      };
    } else {
      options.sourcemapIgnoreList = relativeSourcePath => this.matches(path.resolve(relativeSourcePath));
    }
    return options;
  }
  ...
  makeResult(value, sourcePath, currentMap) {
    ...
    if (value instanceof MagicString && path.isAbsolute(sourcePath)) {
      return {
        code: value.toString(),
        map: SourceMap.merge(currentMap, value.generateMap({
          file: path.basename(sourcePath) + ".map",
          source: sourcePath,
          includeContent: true
        }))
      };
    ...
    if (isFunction(this.options.transform)) {
      ...
      if (transformed !== null) {
        return {
          ...
          fileName: path.relative(path.dirname(entry.id), module.id),
  ...
  makeBundle(ctx, options, modules) {
    if (isString(this.options.output, true)) {
      const path$1 = this.getBundlePath(options);
      if (path$1 === null || !path.isAbsolute(path$1)) {
      ...
      const name = path.basename(path$1, ext);
  ...
  getBundlePath(options) {
    ...
    if (isString(options.dir, true)) {
      path$1 = path.join(options.dir, "bundle.js");
    }
    if (isString(path$1)) {
  ...
  getBundleModules(ctx, bundle) {
    ...
    const isNotBundled = _ref => {
      let {
        id,
        code
      } = _ref;
      return !renderedModules[id] && this.shouldOutput(code);
    };
    ...
    const byImportedOrder = (_ref2, _ref3) => {
      let {
        id: a
      } = _ref2;
      let {
      ...
      return importedIds.indexOf(a) - importedIds.indexOf(b);
    };
    const modules = Array.from(this.#cache.values()).filter(isNotBundled).sort(byImportedOrder);
  ...
  prepareSourceMap(ctx, options, module) {
    if (Boolean(options.sourcemap) && module.map instanceof SourceMap) {
      ...
      const filePath = path.resolve(path.join(dir, module.fileName));
      ...
      module.map.file = path.basename(filePath);
      module.map.sources = module.map.sources.map(source => {
        if (isString(source, true)) {
          if (isString(module.map.sourceRoot, true)) source = path.join(module.map.sourceRoot, source);
          if (isString(source) && source.startsWith("file:")) source = url.fileURLToPath(source);
          if (!path.isAbsolute(source)) source = path.resolve(source);
          const relativeSourcePath = path.relative(dir, source).replace(/[\/\\]/g, "/");
